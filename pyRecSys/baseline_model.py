...
from .recsys_base import RecSysBase

from typing import Tuple


class BaselineModel(RecSysBase):
    """
    Baseline model which models the user-item rating as "r_{ui} = mu + ubias_{u} + ibias_{i}" which is sum of a global mean and the corresponding
    user and item biases.
    - mu: The global mean which is estimated as the mean of all ratings.
    - ubias_{u}: The user biases is estimated as the vectors of length n_users
    - ibias_{i}: The item biases is estimated as the vectors of length i_items
    ...
    Attrs:
        ...
        predictions_possible {list} [after predict only] -- Boolean vector of whether both user and item were known for prediction. Only available after calling predict
    """
    ...
    def update_users(
        ...
        """
        Update user biases vector with new/updated user-item ratings information using SGD.
        Only the user parameters corresponding for the new/updated users will be updated and item parameters will be left alone.
        ...
        if self.method == "sgd":
            ...
            self.user_biases, _, self.train_rmse = _als(
                X=X.to_numpy(),
                global_mean=self.global_mean,
                user_biases=self.user_biases,
                item_biases=self.item_biases,
...
def _sgd(
    ...
    Returns:
        ...
        train_rmse {float} -- Training rmse values
    """
    train_rmse = []
...
def _als(
    ...
    For every epoch, the item biases are held constant while solving directly for the user biases parameters.
    Then the user biases is held constant and the same is done for the item biases.
...
def _calculate_rmse(
    ...
    """
    Calculates RMSE for given data and model parameters
